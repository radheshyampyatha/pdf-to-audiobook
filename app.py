...
VOICE_MAP = {
    'en': [
        ...
        {'key': 'aria2',   'id': 'en-US-GuyNeural',             'label': 'Dylan',   'desc': 'Velvety & Soothing'},
...
def storyteller_transform(text):
    ...
    text = re.sub(r'\s+', ' ', text)
    text = text.strip()
    text = re.sub(r'\bHowever\b', 'But here is where it gets interesting. However', text, flags=re.IGNORECASE)
    text = re.sub(r'\bMeanwhile\b', 'And while that is happening, meanwhile', text, flags=re.IGNORECASE)
    text = re.sub(r'\bSuddenly\b', 'And then, out of nowhere, suddenly', text, flags=re.IGNORECASE)
    text = re.sub(r'\bEventually\b', 'After everything, eventually', text, flags=re.IGNORECASE)
    text = re.sub(r'\bFurthermore\b', 'And if that was not enough, furthermore', text, flags=re.IGNORECASE)
    text = re.sub(r'\bMoreover\b', 'On top of that, moreover', text, flags=re.IGNORECASE)
    text = text.rstrip(' .')
...
async def tts_chunks(chunks, voice_id, output_paths):
    """Run all TTS chunks in a single event loop — faster than asyncio.run() per chunk."""
    tasks = []
    for chunk, outpath in zip(chunks, output_paths):
        tasks.append(_generate_chunk(chunk, voice_id, outpath))
    await asyncio.gather(*tasks)


async def _generate_chunk(text, voice_id, output_path):
...
def convert():
    ...
    try:
        ...
        if ONLINE:
            ...
            asyncio.run(tts_chunks(chunks, voice_info['id'], chunk_paths))
        else:
            chunk_paths = []
        ...
        bitrate = 256000 if ONLINE else 128000
...
if __name__ == '__main__':
    ...
    log.info(f"Network: http://192.168.13.21:{PORT}")
